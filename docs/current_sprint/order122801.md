...
| 단계 | 명령어 | 결과 | 비고 |
|------|--------|------|------|
| 빌드 | `npm run build` | ✅ 성공 | Phase 2 완료 후 |
| 테스트 | `npm test` | ✅ 44 passed, 4 skipped | Phase 3 완료 후 |
| 마이그레이션 | `psql -f migrations/001_multi_provider.sql` | ⏳ 대기 | Phase 5에서 실행 |

## 11. 작업 순서

### Phase 1: 스키마 및 마이그레이션

- [x] 1.1 `database/migrations/` 폴더 생성
- [x] 1.2 `001_multi_provider.sql` 마이그레이션 스크립트 작성
- [x] 1.3 `database/schema.sql` 업데이트
- [x] **검증**: 마이그레이션 스크립트 문법 확인

### Phase 2: Backend 코드 변경

- [x] 2.1 `queries.ts` - AuthProvider 타입 및 User 인터페이스 변경
- [x] 2.2 `queries.ts` - `findUserByProvider()` 함수 추가
- [x] 2.3 `queries.ts` - `findUserByGoogleId()` ~~deprecated 처리~~ **완전 삭제**
- [x] 2.4 `queries.ts` - `createUser()` 시그니처 변경
- [x] 2.5 `passport.ts` - Google Strategy 코드 수정
- [x] 2.6 `express.d.ts` - 타입 정의 업데이트
- [x] **검증**: `npm run build` 성공

### Phase 3: 테스트 업데이트

- [x] 3.1 기존 테스트 수정 (queries 관련)
- [x] 3.2 새 함수 테스트 추가
- [x] **검증**: `npm test` 성공 (44 passed, 4 skipped)

### Phase 4: 문서화

- [x] 4.1 `MIGRATION_GUIDE.md` 작성
- [x] 4.2 `BACKEND_STRUCTURE.md` 업데이트
- [x] **검증**: 문서 리뷰

### Phase 5: 로컬 마이그레이션 테스트 (사용자 수행)
...
> **Update (2025-12-29)**: `findUserByGoogleId()` 함수는 v2.0.0에서 **완전 삭제**되었습니다.
> 모든 코드에서 `findUserByProvider('google', googleId)`를 직접 사용하세요.
...
- [x] Multi-Provider OAuth 스키마 설계 및 마이그레이션 스크립트
- [x] Backend 코드 리팩토링 (`google_id` → `provider` + `provider_id`)
- [x] deprecated 함수 제거 (`findUserByGoogleId`)
- [x] 테스트 업데이트 (44 passed)
- [x] 문서화 (MIGRATION_GUIDE.md, BACKEND_STRUCTURE.md)

### 사용자 액션 필요

- [ ] Phase 5: 로컬 DB 마이그레이션 테스트
